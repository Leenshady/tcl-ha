    ...
    async def async_step_user(self, user_input: dict[str, Any] | None = None) -> FlowResult:
        ...
        if user_input is not None:
            try:
                ...
                client = TclClient(self.hass, user_input[ACCOUNT_ID], '')
                await client.initialize()
                token_info = await client.refresh_token(user_input[REFRESH_TOKEN])
                # 获取用户信息
                client = TclClient(self.hass, user_input[ACCOUNT_ID], token_info.token)
                await client.initialize()
                user_info = await client.get_user_info()

                return self.async_create_entry(title="Tcl - {}".format(user_info['mobile']), data={
                    'account': {
